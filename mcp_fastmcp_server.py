...
def generate_full_markdown_report(job_id: str, duration: str, results: dict, path: str) -> str:
    ...
    if git_info.get("status") == "clean":
        md.append(f"**Branch:** `{git_info.get('current_branch', 'unknown')}`")
        md.append(f"**Recent Commits:** {git_info.get('total_commits', 0)}")
        
        if git_info.get("recent_commits"):
            md.append("")
            md.append("**Latest Changes:**")
            for commit in git_info.get("recent_commits", [])[:5]:
                md.append(f"- {commit}")
        
        if git_info.get("uncommitted_changes"):
            md.append("")
            md.append("⚠️ **Uncommitted Changes:**")
            for change in git_info.get("uncommitted_changes", [])[:10]:
                md.append(f"- {change}")
    else:
