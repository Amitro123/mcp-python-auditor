...
class ScoreBreakdown:
    ...
    def final_score(self) -> int:
        return max(0, self.base_score - self.security_penalty - 
                   self.quality_penalty - self.testing_penalty - 
...
class ScoringEngine:
    ...
    def calculate_score(audit_results: Dict[str, Any]) -> ScoreBreakdown:
        ...
        if bandit.get("total_issues", 0) > 0:
            breakdown.security_penalty = min(
                bandit["total_issues"] * 5, 30
            )
        ...
        exact_dups = [d for d in duplication.get("duplicates", []) 
                      if d.get("similarity", 0) > 95]
