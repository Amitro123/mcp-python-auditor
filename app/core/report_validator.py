...
class ReportValidator:
    ...
    def validate_consistency(self, json_data: dict[str, Any], markdown_report: str, score_breakdown: ScoreBreakdown) -> list[str]:
        ...
        if md_score and abs(score_breakdown.final_score - md_score) > 2:
            errors.append(f"Score mismatch: Calculated={score_breakdown.final_score}, Report={md_score}")
        ...
        if md_coverage is not None and json_coverage != md_coverage:
            errors.append(f"Coverage mismatch: JSON={json_coverage}%, Report={md_coverage}%")
        ...
        if json_coverage < 30:
            ...
            for phrase in misleading_phrases:
                if phrase.lower() in markdown_report.lower():
                    errors.append(f"Misleading phrase '{phrase}' found for {json_coverage}% coverage")
        ...
        if md_security is not None and md_security != total_security:
            errors.append(f"Security count mismatch: JSON={total_security}, Report={md_security}")
        ...
        if md_dead is not None and md_dead != json_dead:
            errors.append(f"Dead code mismatch: JSON={json_dead}, Report={md_dead}")

        return errors

    def _extract_score(self, markdown: str) -> int | None:
    ...
    def _extract_coverage(self, markdown: str) -> float | None:
    ...
    def _extract_security_count(self, markdown: str) -> int | None:
    ...
    def _extract_dead_code_count(self, markdown: str) -> int | None:
