...
class FileFilter:
    ...
    TOOL_CONFIGS = {
        ...
        "ruff": {"include": ["**/*.py"], "exclude": [], "description": "Lint all Python files"},
        "tests": {
    ...
    def get_filtered_files(self, tool_name: str) -> list[Path]:
        ...
        logger.info(f"FileFilter[{tool_name}]: Found {total_found} files, filtered to {len(files)} ({total_excluded} excluded)")
